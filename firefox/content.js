...
function requestPreviewWindow(username) {
	browser.runtime.sendMessage({ type: "xbuddy:update-preview", username }, (response) => {
		if (browser.runtime.lastError) {
			console.warn("X Buddy preview message failed", browser.runtime.lastError);
			return;
		}
...
function tryCaptureAboutLocation(username) {
	...
	browser.runtime.sendMessage({ type: "xbuddy:store-location", username, location }, () => {
		if (browser.runtime.lastError) {
...
function resolveLocationDisplay(location) {
	...
	const flagUrl = browser.runtime.getURL(`assets/flags/${code}.svg`);
...
function ensureLocationForUsername(username, allowFetch = true) {
	...
	browser.runtime.sendMessage({ type: "xbuddy:get-location", username }, (response) => {
		pendingLocationLookups.delete(username);
		if (browser.runtime.lastError) {
			console.warn("X Buddy cache lookup failed", browser.runtime.lastError);
			return;
		}
		if (!response?.ok) return;
...
function initialisePreferenceSync() {
	browser.storage.sync.get(
		...
		(data) => {
			if (browser.runtime.lastError) {
				console.warn("X Buddy preference read failed", browser.runtime.lastError);
				return;
			}
